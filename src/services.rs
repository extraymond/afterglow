...
pub struct Registry {
    listeners: Rc<Mutex<Vec<Sender<Box<dyn Service>>>>>,
}

#[async_trait(?Send)]
pub trait Service {
    async fn register(&self, registry: &mut Registry) -> Receiver<Box<dyn Service>> {
        let mut txs = registry.listeners.lock().await;
